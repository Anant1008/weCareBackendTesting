...
const uploadReport = async(req, res, next) => {
    ...
    if (existingReports) {
        ...
        //Testing
        return res.status(200).json({message : "Added to reports array"});
    } else {
        ...
        //Testing 
        return res.status(201).json({message: "New report uploaded"});
    }
};
...
const getAllReports = async(req, res, next) => {
    try {
        ...
        //Testing
        res.status(200).json({message: "Reports Array"});
...
const deleteReport = async(req, res, next) => {
    try {
        ...
        const email = req.params.email;
        const record = null;      //remove this line after testing
        // const record = await Report.findOne({ patientEmail: email });
        if (record) {
            // let reportsArray = record.filename;
...
const downloadReport = async(req, res, next) => {
    ...
    return res.status(201).send('reports.jpg');
